...
namespace Test
{
    ...
    public class SerializerTest
    {
        ...
        public void AnimationsXml_ToJson()
        {
            ...
            var json = new Node(JsonTree.Node.Type.List);
            ...
            foreach (XmlNode node in nodes) {
                if (keys.ContainsKey(node.Name)) {
                    var dict = new Node(JsonTree.Node.Type.Dictionary);
                    foreach (var key in keys[node.Name]) {
                        var attr = node.Attributes.GetNamedItem(key);
                        if (attr != null) {
                            dict.AsDictionary.Add(attr.Name, new Node(JsonTree.Node.Type.String, attr.Value));
                        }
                    }
                    if (node.Name == "sequence") {
                        var src = node.FirstChild.Attributes["src"].Value;
                        dict.AsDictionary.Add("src", new Node(JsonTree.Node.Type.String, src));
                    }
                    var item = new Node(JsonTree.Node.Type.Dictionary);
...
";
            ...
            // Act
            root.List.ElementAt(2).Dictionary.Add("new child", new Node(JsonTree.Node.Type.String) { Value = "new string" });
