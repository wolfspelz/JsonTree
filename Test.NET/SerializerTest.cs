...
namespace Test
{
    ...
    public class SerializerTest
    {
        ...
        public void AnimationsXml_ToJson()
        {
            ...
            var json = new Node(Node.Type.List);
            ...
            foreach (XmlNode node in nodes) {
                if (keys.ContainsKey(node.Name)) {
                    var dict = new Node(Node.Type.Dictionary);
                    foreach (var key in keys[node.Name]) {
                        var attr = node.Attributes.GetNamedItem(key);
                        if (attr != null) {
                            dict.AsDictionary.Add(attr.Name, new Node(Node.Type.String, attr.Value));
                        }
                    }
                    if (node.Name == "sequence") {
                        var src = node.FirstChild.Attributes["src"].Value;
                        dict.AsDictionary.Add("src", new Node(Node.Type.String, src));
                    }
                    var item = new Node(Node.Type.Dictionary);
